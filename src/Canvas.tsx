...
const Canvas = ({ isFullscreenActive = false }: CanvasProps) => {
  ...
  }, [isGameMode, isFullscreenActive]);
  
  // Update isFullscreen state when the isFullscreenActive prop changes
  useEffect(() => {
  ...
  const canvasContainerStyle = isFullscreenActive && isGameMode
    ? {
  ...
  const canvasStyle = isFullscreenActive && isGameMode
    ? {
        width: '100vw',
        height: '100vh',
        ...backgroundStyle,
  ...
  return (
    <div ref={parentRef} className={`flex-1 flex flex-col h-full ${isFullscreenActive && isGameMode ? 'fullscreen-canvas' : ''}`}>
      <div className={`flex-1 flex items-center justify-center ${isFullscreenActive && isGameMode ? 'p-0' : 'p-4'} canvas-workspace relative`}>
        <div className="canvas-container" style={canvasContainerStyle}>
          <div
            ref={containerRef}
            className={`relative shadow-lg ${!isFullscreenActive && 'rounded-lg'} ${!isGameMode && isDraggingOver ? 'ring-2 ring-primary' : ''}`}
            style={canvasStyle}

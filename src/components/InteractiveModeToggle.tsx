...
const InteractiveModeToggle = ({ isMobileNavigation }: InteractiveModeToggleProps = {}) => {
  ...
  if (isMobile && isMobileNavigation) {
    return (
      ...
      >
        {isInteractiveMode ? (
          <Zap className="h-6 w-6 text-canvas-purple" />
        ) : (
          <Pencil className="h-6 w-6 text-gray-500" />
        )}

  ...
  return (
    <>
      <div 
        ...
        onClick={handleClick}
      >
        <MousePointer className="w-8 h-8 text-green-700 mb-2" />
        <p className="text-sm font-medium text-center text-gray-700 line-clamp-2">
          {clickSequencePuzzleConfig.name}
        </p>
        <div className="text-xs text-gray-500 mt-1">
          {language === 'en' ? 'Click Sequence Puzzle' : 'פאזל לחיצה לפי סדר'}: {clickSequencePuzzleConfig.images.length} {language === 'en' ? 'images' : 'תמונות'}
        </div>
      </div>

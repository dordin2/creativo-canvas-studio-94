...
export const LibraryModal = () => {
  ...
  const fetchLibraryElements = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('library_elements').select('*').order('created_at', {
        ascending: false
      });
  ...
  const handleDeleteElement = async (elementId: string) => {
    try {
      ...
      const {
        error: storageError
      } = await supabase.storage.from('library_elements').remove([filePath]);
      if (storageError) throw storageError;
      const {
        error: dbError
      } = await supabase.from('library_elements').delete().eq('id', elementId);
      ...
      <SheetContent side="left" className="w-[400px] sm:w-[540px]">
        ...
        <div className="mt-4">
          {isLoading ? <div className="flex items-center justify-center h-32">
            ...
            </div> : elements.length === 0 ? <div className="text-center py-8 text-gray-500">
              No elements found
            </div> : <div className="grid grid-cols-2 gap-4">
              {elements.map(element => <div key={element.id} className="border rounded-lg p-2 hover:bg-gray-50 transition-colors relative cursor-pointer" onClick={() => handleImageClick(element)}>
                  <img src={element.image_path} alt={element.name} className="w-full h-32 object-contain bg-gray-50 rounded-md" />
                  <p className="mt-2 text-sm font-medium truncate">{element.name}</p>
                  <p className="text-xs text-gray-500">
                    {new Date(element.created_at).toLocaleDateString()}
                  </p>
                  {isAdmin && <Button variant="destructive" size="icon" className="absolute top-2 right-2" onClick={e => {
              e.stopPropagation();
